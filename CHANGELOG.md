...
- 首页、栏目、标签、作者和归档的第 2 页及以后现在使用带页码的 canonical 与标题，并输出 `rel="prev"` / `rel="next"`，避免分页被收成首页重复内容。
- About 模板与页头、首页资料卡使用同一套方形 / 横向 Logo 回退，不再只读取 Halo 站点 Logo。
- Organization 结构化数据的 `@id` 统一为去掉尾斜杠的站点地址加 `#organization`，独立页不再因 `site.url` 是否带 `/` 拆成两个实体。
- 文章分享按钮在服务端直接输出各平台 URL，无脚本时不再落到页顶的 `#`。

## [0.9.3] - 2026-08-21

### 修复

- 首页中文界面的侧栏栏目标题由“内容栏目”调整为更简洁的“分类”，英文界面继续显示 `Categories`。

## [0.9.2] - 2026-08-21

### 文档

- 重写中文 README，按安装、Logo 上传、主题设置、插件边界和开发验证重新组织内容。
- 新增独立的 `README.en.md`，提供与中文文档对应的英文安装和配置说明。
- 明确 Logo 上传入口位于「主题设置 → 外观」，并区分前台 Logo 与 SEO 结构化数据使用的组织 Logo。

## [0.9.1] - 2026-08-21

### 修复

- 在「外观」设置中补充 `1:1` 方形 Logo 与 `3:1` 横向 Logo 的直接上传控件，用户选择展示方式后即可上传对应图片，不再依赖不明确的 Halo 站点 Logo 入口。
- 横向 Logo 未上传时回退为方形 Logo 与站点标题；方形 Logo 留空时继续兼容 Halo 站点 Logo 与主题默认图标。

## [0.9.0] - 2026-08-21

### 新增

- Logo 新增 `1:1` 方形图标加站点标题与 `3:1` 横向图片两种展示方式，横向模式适合使用已包含文字的 Logo。
- 页头与首页资料卡统一使用所选 Logo 模式；横向模式未上传站点 Logo 时自动回退到方形图标与站点标题。

## [0.8.1] - 2026-08-20

### 修复

- 修复体积预算检查在 ZIP 写入完成后执行的问题，超出预算时现在会清理不合规产物。
- 修复 legacy asset aliases 从完整代码改为轻量转发后可能导致浏览器缓存旧内容的问题，HTML 引用现在会添加版本号查询参数。
